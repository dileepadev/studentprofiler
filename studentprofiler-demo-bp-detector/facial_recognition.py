...
video_path = "/studentprofiler-demo-bp-detector/videos/group/1.mp4"
    ...
    for (top, right, bottom, left), face_encoding in zip(face_locations, face_encodings):
        ...
        for student_name, student_face_encoding in student_face_encodings.items():
            match = face_recognition.compare_faces(
                [student_face_encoding], face_encoding)
            if match[0]:
        ...
        cv2.rectangle(resized_frame, (left, top),
                      (right, bottom), (0, 255, 0), 2)

        # Draw black background with white text for student's name
        label_width = cv2.getTextSize(
            name, cv2.FONT_HERSHEY_SIMPLEX, 0.6, 2)[0][0]
        cv2.rectangle(resized_frame, (left, top - 30),
                      (left + label_width + 10, top), (0, 0, 0), -1)
        cv2.putText(resized_frame, name, (left + 5, top - 10),
