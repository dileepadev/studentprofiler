...
while True:
    ...
    for idx, teacher_image in enumerate(teacher_images):
        ...
        result = cv2.matchTemplate(
            gray_frame, teacher_image, cv2.TM_CCOEFF_NORMED)
    ...
    if best_match_idx is not None:
        ...
        for pt in zip(*locations[::-1]):
            ...
            cv2.rectangle(
                frame, pt, (pt[0] + overlay_width, pt[1] + overlay_height), (0, 255, 0), 2)
            cv2.putText(frame, f"Teachers {best_match_idx + 1}",
                        (pt[0], pt[1] - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)
